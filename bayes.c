...
void ChangeOperation(int *VectCoevComb, double *S, double *D, double *W1,
		double *W2, double *U) {
	int loop_j = 0;
	...
	*BW2 = *AW2;
	...
	while (iteration < innerLoopIteration) {
		...
		*BW2 = *AW2;
		...
		for (loop_j = 0; loop_j < nrComb; loop_j++) {
			BVectorComb[loop_j] = AVectorComb[loop_j];
		}
		*U = random() / (RAND_MAX + 1.0);
		...
		if ((((BLogLikelihood + BPrior) - (ALogLikelihood + APrior))
				* temperature) + log(*U) >= log(x)) {
...
int bayes() {
	...
	fp = fopen(outFile, "w+");
	fprintf(fp,
			"iteration\tPosterior\tLogLikelihood\tPrior\tS\tD\tR1\tR2\tAA\tAC\tAG\tAT\tCA\tCC\tCG\tCT\tGA\tGC\tGG\tGT\tTA\tTC\tTG\tTT\n");
	fflush(fp);
	/******* FOR 4 CHAINS***********/
	//////THREAD1
	th1S = s;
	...
	while (iteration < IT) {
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			argThread1[loop_i] = th1VectorComb[loop_i - 9];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			argThread2[loop_i] = th2VectorComb[loop_i - 9];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			argThread3[loop_i] = th3VectorComb[loop_i - 9];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			argThread4[loop_i] = th4VectorComb[loop_i - 9];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			th1VectorComb[loop_i - 9] = (int) argThread1[loop_i];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			th2VectorComb[loop_i - 9] = (int) argThread2[loop_i];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			th3VectorComb[loop_i - 9] = (int) argThread3[loop_i];
		}
		...
		for (loop_i = 9; loop_i < nrComb + 9; loop_i++) {
			th4VectorComb[loop_i - 9] = (int) argThread4[loop_i];
		}
		...
		double equationValue1 = (((th2LogLikelihood + th2Prior) * temperature1)
				+ ((th1LogLikelihood + th1Prior) * temperature2))
				- (((th1LogLikelihood + th1Prior) * temperature1)
						+ ((th2LogLikelihood + th2Prior) * temperature2));

		double equationValue2 = (((th3LogLikelihood + th3Prior) * temperature1)
				+ ((th1LogLikelihood + th1Prior) * temperature3))
				- (((th1LogLikelihood + th1Prior) * temperature1)
						+ ((th3LogLikelihood + th3Prior) * temperature3));

		double equationValue3 = (((th4LogLikelihood + th4Prior) * temperature1)
				+ ((th1LogLikelihood + th1Prior) * temperature4))
				- (((th1LogLikelihood + th1Prior) * temperature1)
						+ ((th4LogLikelihood + th4Prior) * temperature4));
		...
		case 0:
			...
			if (equationValue1 >= log(x)) {
				...
				tmpW1 = th1W1;
				tmpW2 = th1W2;
				tmpLikelihood = th1Likelihood;
				tmpLogLikelihood = th1LogLikelihood;
				tmpPrior = th1Prior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					tmpVectorComb[loop_j] = th1VectorComb[loop_j];
				}
				...
				th1W1 = th2W1;
				th1W2 = th2W2;
				th1Likelihood = th2Likelihood;
				th1LogLikelihood = th2LogLikelihood;
				th1Prior = th2Prior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					th1VectorComb[loop_j] = th2VectorComb[loop_j];
				}
				...
				th2W1 = tmpW1;
				th2W2 = tmpW2;
				th2Likelihood = tmpLikelihood;
				th2LogLikelihood = tmpLogLikelihood;
				th2Prior = tmpPrior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					th2VectorComb[loop_j] = tmpVectorComb[loop_j];
				}
				//  printf("\nEND SWAP 0\n");
			} else {
		...
		case 1:
			...
			if (equationValue2 >= log(x)) {
				...
				th1W1 = th3W1;
				th1W2 = th3W2;
				th1Likelihood = th3Likelihood;
				th1LogLikelihood = th3LogLikelihood;
				th1Prior = th3Prior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					th1VectorComb[loop_j] = th3VectorComb[loop_j];
				}
				...
				th3W1 = tmpW1;
				th3W2 = tmpW2;
				th3Likelihood = tmpLikelihood;
				th3LogLikelihood = tmpLogLikelihood;
				th3Prior = tmpPrior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					th3VectorComb[loop_j] = tmpVectorComb[loop_j];
				}
				//  printf("\nEND SWAP 1\n");
		...
		case 2:
			...
			if (equationValue3 >= log(x)) {
				...
				th1W1 = th4W1;
				th1W2 = th4W2;
				th1Likelihood = th4Likelihood;
				th1LogLikelihood = th4LogLikelihood;
				th1Prior = th4Prior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					th1VectorComb[loop_j] = th4VectorComb[loop_j];
				}
				...
				th4W1 = tmpW1;
				th4W2 = tmpW2;
				th4Likelihood = tmpLikelihood;
				th4LogLikelihood = tmpLogLikelihood;
				th4Prior = tmpPrior;
				for (loop_j = 0; loop_j < nrComb; loop_j++) {
					th4VectorComb[loop_j] = tmpVectorComb[loop_j];
				}
				// printf("\nEND SWAP 2\n");
		...
		if ((iteration % sample_freq == 0) & (iteration >= burnin)) {
			fprintf(fp, "%d\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t", iteration, post,
					th1LogLikelihood, th1Prior, th1S, th1D, th1W1, th1W2);
			for (loop_j = 0; loop_j < nrComb; loop_j++) {
				fprintf(fp, "%d\t", th1VectorComb[loop_j]);
			}
			fprintf(fp, "\n");
			fflush(fp);
		}
